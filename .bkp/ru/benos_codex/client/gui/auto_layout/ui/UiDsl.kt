...
class UiBuilder {
    ...
    fun box(
        modifier: UiModifier = UiModifier.None,
        backgroundTexture: UiTexture? = null,
    ...
    fun centerNode(
        modifier: UiModifier = UiModifier.None,
        horizontalCenterize: Boolean = true,
        verticalCenterize: Boolean = true,
        block: UiBuilder.() -> Unit
    ) {
        children.add(
            UiCenterNode(
                modifier = modifier,
                horizontalCenterize = horizontalCenterize,
                verticalCenterize = verticalCenterize,
                child = UiBox(
                    UiModifier.None,
                    children = UiBuilder().apply(block).build()
                )
     ...
     * @param verticalAlign Vertical alignment inside the allocated bounds.
    ...
    fun label(
        ...
        verticalAlign: UiTextVerticalAlign = UiTextVerticalAlign.Top,
        wrap: Boolean = false,
    ...
    fun button(
        ...
        modifier: UiModifier = UiModifier.None,
        key: String? = null,
        textures: UiButtonTextures? = null,
        tooltip: UiTooltip? = null,
        onClick: () -> Unit
    ) {
        children.add(
            if (key != null) UiButton(text, onClick, modifier, key = key, textures = textures, tooltip = tooltip)
            else UiButton(text, onClick, modifier, textures = textures, tooltip = tooltip)
        )
    }

    fun textureButton(
        text: Component,
        textures: UiButtonTextures,
        modifier: UiModifier = UiModifier.None,
        key: String? = null,
        tooltip: UiTooltip? = null,
        onClick: () -> Unit
     ...
     *
     * @param selected Currently selected value. Used only for label/highlight state.
     * @param options Available options shown in the popup list.
     * @param onSelect Invoked when the user picks an option.
     * @param modifier Widget sizing and padding rules.
    ...
    fun <T> dropdown(
        ...
        onSelect: (UiDropdownOption<T>) -> Unit,
        modifier: UiModifier = UiModifier.None,
        key: String = "dropdown:${options.hashCode()}",
        tooltip: UiTooltip? = null
    ) {
        children.add(
            UiDropdown(
