...
class SettingsScreen: AutoLayoutScreen("$MOD_ID.gui.settings.labels.title".mtranslate(MOD_ID)) {
    companion object {
        fun show() =
        ...
        splitRow(
            ...
            left = {
                column(gap = 4, modifier = UiModifier().fillWidth()) {
                    ...
                    categoryColumn("menus", ::groupMenusExpanded) {
    ...
    private val String.k: Component get() =
        "$MOD_ID.gui.settings.${this@k}".mtranslate
    ...
    private fun UiBuilder.categoryColumn(id: String, expander: KMutableProperty0<Boolean>, block: UiBuilder.() -> Unit) =
        category(id, expander) {
            column(gap = 4, block = block)
        }
}
