...
class UiRuntime(
    ...
    fun click(mouseX: Double, mouseY: Double): Boolean {
        ...
        val dragHit = dragRegions.lastOrNull { it.rect.contains(mouseX, mouseY) }
        if (dragHit != null) {
        ...
        val hit = clickRegions.lastOrNull { it.rect.contains(mouseX, mouseY) } ?: return false
    ...
    fun mouseScrolled(scrollY: Double, isCtrlDown: Boolean): Boolean =
        if (
            activeDropdownKey
                ?.takeIf { key ->
                    isToggled(key) && (dropdownRegions[key]?.any { it.contains(mouseX.toDouble(), mouseY.toDouble()) } == true)
                }
                ?.let { key -> dropdownInputs[key]?.mouseScrolled(scrollY, isCtrlDown) } == true
        ) {
            true
        } else if (
            focusedTextInputKey
                ?.takeIf { key -> textInputRects[key]?.contains(mouseX.toDouble(), mouseY.toDouble()) == true }
                ?.let { key -> textInputStates[key]?.input?.mouseScrolled(scrollY, isCtrlDown) } == true
        ) {
            true
        } else {
            val hovered = scrollRegions.lastOrNull { it.rect.contains(mouseX.toDouble(), mouseY.toDouble()) } ?: return false
    ...
    private data class ClickRegion(val rect: UiRect, val onClick: (mouseX: Double, mouseY: Double) -> Unit)
    ...
    private data class ScrollRegion(val rect: UiRect, val onScroll: (scrollY: Double) -> Unit)
...
interface UiTextInput {
    fun charTyped(codePoint: Char): Boolean
    fun keyPressed(keyCode: Int, isCtrlDown: Boolean, isShiftDown: Boolean): Boolean
    fun mouseScrolled(scrollY: Double, isCtrlDown: Boolean): Boolean = false
}
...
/** Item rendering mode used by [UiItemStackView] and [UiItemStackRenderer]. */
...
/** Per-frame timing context passed into item-stack transform callbacks. */
data class UiItemStackFrame(
...
/** 3D transform applied to item-stack preview rendering. */
data class UiItemStackTransform(
...
class UiDropdown<T>(
    ...
    override val modifier: UiModifier = UiModifier.None,
    private val key: String = "dropdown:${value.name}",
    private val tooltip: UiTooltip? = null
    ...
    override fun render(runtime: UiRuntime, bounds: UiRect) {
        ...
        val currentOption = options.firstOrNull { it.value == value.get() }
        val currentLabel = currentOption?.label ?: Component.literal(value.get().toString())
        val currentIndex = options.indexOfFirst { it.value == value.get() }.coerceAtLeast(0)
        ...
        runtime.addOverlayRenderer { _, screenHeight ->
            ...
            runtime.addClickRegion(dropdownRect) { _, _ -> }
            ...
            runtime.addScrollRegion(dropdownRect) { scrollY ->
                handleDropdownScroll(scrollY, optionHeight, contentRect.height, state)
            }
            ...
            options.forEachIndexed { index, option ->
                ...
                val optionHovered = optionRect.contains(runtime.mouseX.toDouble(), runtime.mouseY.toDouble())
                if (optionHovered) {
                    state.highlightedIndex = index
                }
                val selected = option.value == value.get()
                val highlighted = state.highlightedIndex == index
                if (optionHovered || highlighted || selected) {
                    runtime.guiGraphics.fill(
                        optionRect.x + 1,
                        optionRect.y + 1,
                        optionRect.right - 1,
                        optionRect.bottom - 1,
                        when {
                            selected -> Theme.buttonFillPressed
                ...
                runtime.addClickRegion(optionRect) { _, _ ->
            ...
            if (!scrollable) return@addOverlayRenderer

            val scrollbarRect = UiRect(contentRect.right + scrollbarGap, dropdownRect.y, scrollbarWidth, dropdownRect.height)
            ...
            val thumbRect = UiRect(scrollbarRect.x, thumbY, scrollbarWidth, thumbHeight)
            val thumbHovered = thumbRect.contains(runtime.mouseX.toDouble(), runtime.mouseY.toDouble())
            ...
            runtime.addClickRegion(scrollbarRect) { _, mouseY ->
    ...
    override fun mouseScrolled(scrollY: Double, isCtrlDown: Boolean): Boolean {
        if (options.isEmpty()) return false
        val runtime = UiRuntime.currentRuntime ?: return false
        if (!runtime.isToggled(key)) return false
...
class UiButton(
    ...
    override val modifier: UiModifier = UiModifier.None,
    private val key: String = "button:${text.string}",
    private val tooltip: UiTooltip? = null
) : UiNode {
    override fun measure(runtime: UiRuntime, maxWidth: Int, maxHeight: Int): UiSize {
    ...
    override fun render(runtime: UiRuntime, bounds: UiRect) {
        ...
        runtime.addDragRegion(
            rect = bounds,
            onStart = { _, _ -> runtime.setPressed(key, true) },
            onDrag = { mouseX, mouseY -> runtime.setPressed(key, bounds.contains(mouseX, mouseY)) },
            onEnd = { mouseX, mouseY ->
                val shouldClick = bounds.contains(mouseX, mouseY)
                runtime.setPressed(key, false)
...
class UiMultilineTextField(
    ...
    override fun render(runtime: UiRuntime, bounds: UiRect) {
        ...
        val gutterRect = UiRect(editorRect.x, editorRect.y, lineNumberWidth, editorRect.height)
        val separatorX = gutterRect.right + 3
        val textRect = UiRect(
            separatorX + 6,
            editorRect.y,
            (editorRect.right - (separatorX + 6)).coerceAtLeast(0),
            editorRect.height
        )
        latestTextRect = textRect
        latestBounds = bounds
        runtime.guiGraphics.fill(separatorX, editorRect.y, separatorX + 1, editorRect.bottom, Theme.separator)
        ...
        if (current.isEmpty() && placeholder != null) {
            runtime.guiGraphics.pose().pushMatrix()
            runtime.guiGraphics.pose().translate(textRect.x.toFloat(), (textRect.y - state.scrollOffset).toFloat())
            ...
            lines.forEachIndexed { index, line ->
                ...
                drawHighlightedLine(runtime, line, textRect.x, y, zoom)
            }
        }
        runtime.guiGraphics.disableScissor()
        ...
        if (focused) {
            ...
            val caretX = textRect.x + (runtime.font.width(caretPrefix) * zoom).toInt()
        ...
        runtime.addClickRegion(bounds) { mouseX, mouseY ->
            runtime.focus(key)
            state.caretIndex = caretIndexForPoint(runtime, value.get(), textRect, mouseX.toInt(), mouseY.toInt(), state.scrollOffset)
        ...
        runtime.addDragRegion(
            ...
            onStart = { mouseX, mouseY ->
                runtime.focus(key)
                val caret = caretIndexForPoint(runtime, value.get(), textRect, mouseX.toInt(), mouseY.toInt(), state.scrollOffset)
    ...
    override fun charTyped(codePoint: Char): Boolean {
        ...
        val normalized = if (codePoint == '\t') ' ' else codePoint
        val current = value.get()
        if (current.length >= maxLength) return true
        if (normalized != '\n' && normalized == 127.toChar()) return false
        val state = runtimeState() ?: return false
        ...
        val candidate = base.substring(0, insertIndex) + normalized + base.substring(insertIndex)
    ...
    private fun caretIndexForPoint(runtime: UiRuntime, text: String, textRect: UiRect, mouseX: Int, mouseY: Int, scrollOffset: Int): Int {
        ...
        val localX = (mouseX - textRect.x).coerceAtLeast(0)
        var column = targetLine.length
        for (index in targetLine.indices) {
            val previousWidth = (runtime.font.width(targetLine.take(index)) * zoom).toInt()
            val nextWidth = (runtime.font.width(targetLine.take(index + 1)) * zoom).toInt()
    ...
    private fun drawSelection(
        ...
        lines.indices.forEach { lineIndex ->
            ...
            val startX = textRect.x + (runtime.font.width(lines[lineIndex].take(startColumn)) * zoom).toInt()
            val endX = textRect.x + (runtime.font.width(lines[lineIndex].take(endColumnExclusive)) * zoom).toInt()
    ...
    private fun drawHighlightedLine(runtime: UiRuntime, line: String, x: Int, y: Int, zoom: Float) {
        ...
        while (start < line.length) {
            ...
            val segmentX = x + (runtime.font.width(line.substring(0, start)) * zoom).toInt()
            runtime.guiGraphics.pose().pushMatrix()
            runtime.guiGraphics.pose().translate(segmentX.toFloat(), y.toFloat())
            runtime.guiGraphics.pose().scale(zoom, zoom)
            runtime.guiGraphics.drawString(runtime.font, segment, 0, 0, color)
            runtime.guiGraphics.pose().popMatrix()
            start = end
        }
    }
